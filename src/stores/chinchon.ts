...
import { ChinchonPlayer } from '@/common/types'
import { create } from 'zustand'
import { persist, createJSONStorage } from 'zustand/middleware'

type State = {
  players: ChinchonPlayer[]
  roundInputs: Record<ChinchonPlayer['id'], number>
}

type Action = {
  addPlayer: (player: ChinchonPlayer) => void
  removePlayer: (id: string) => void
  resetGame: () => void
  removeLastRound: () => void
  addRound: () => void,
  resetInputs: () => void,
  setRoundInput: (input: Record<ChinchonPlayer['id'], number>) => void
}

export const useChinchon = create<State & Action>()(persist(
  (set) => ({
    players: [],
    roundInputs: {},
    ...
    resetGame: () => set({
      players: [...GAME_CONFIG.CHINCHON.INITIAL_STATE.players],
      roundInputs: GAME_CONFIG.CHINCHON.INITIAL_STATE.roundInputs,
    }),
